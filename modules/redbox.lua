...
local Martyrdom = {Enabled = false}
Martyrdom = Blatant.CreateOptionsButton({
    Name = "Martyrdom",
    Function = function(callback) end,
    HoverText = "Drop a live grenade when killed."
})
...
OldNamecall = hookmetamethod(game, "__namecall", function(instance, ...)
    ...
    if not checkcaller() and nounequip and method == "UnequipTools" and instance.Parent == LocalPlayer.Character and instance.Name == "Humanoid" then
        return
    end
    return OldNamecall(instance,...)
end)

local ThrowGrenade = ReplicatedStorage.Assets.Remotes.throwGrenade
local GrenadeListener = function()
    noequip = false
    local Backpack = LocalPlayer:FindFirstChildWhichIsA("Backpack")
    local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
    LocalPlayer.Character:WaitForChild("Head", 10)
    local LastExplosion = -20
    Humanoid.HealthChanged:Connect(function(health)
        if Martyrdom.Enabled and health <= 4 and os.clock() - LastExplosion > 9 then
            LastExplosion = os.clock()
            noequip = true
            if LocalPlayer.Character and Backpack then
                local num = 66
                for i = 1, num / 2 do
                    ThrowGrenade:FireServer(LocalPlayer.Character, Loadout.Misc:FindFirstChild("Frag Grenade"))
                end
                    ...
                    coroutine.wrap(function()
                        if v.Name == "Frag Grenade" then
                            ...
                            for _ = 1, 1600 do
                                ...
                                if v and v.Parent == LocalPlayer.Character then
                                    ThrowGrenade:FireServer(Vector3.new(Random.new():NextNumber(-20, 20), Random.new():NextNumber(-20, 20), Random.new():NextNumber(-20, 20)), v)
                                end
                                wait(0.01)
                            end
                        end
...
GrenadeListener()
LocalPlayer.CharacterAdded:Connect(GrenadeListener)
